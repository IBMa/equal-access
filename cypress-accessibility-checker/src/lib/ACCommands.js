...
let ACCommands = module.exports = {
     ...
     */
    isLabelUnique: (label) => {
        ACCommands.DEBUG && console.log("START 'aChecker.isLabelUnique' function");

        // Variable Decleration
        let labelExists = false;

        ACCommands.DEBUG && console.log("Checking if label: " + label + " is unique.");
        ...
        labelExists = ACCommands.scanSummary.pageScanSummary.some(function (scanSummary) {
            return scanSummary.label === label;
        });

        ACCommands.DEBUG && console.log("END 'aChecker.isLabelUnique' function");
    ...
    getBaseline: function (label) {
        try {
            return require(path.join(path.join(process.cwd(), ACCommands.Config.baselineFolder), label));
        } catch (e) {
            return null;
        }
    },
