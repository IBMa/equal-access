...
const MultiScanReport = require("./multiScanReport");
const MultiScanData = require("./multiScanData");
 ...
 * @memberOf this
 */
var ACReporterXLSX = function (aChecker) {

    this.preprocessReport = (report, filter, scroll) => {
        const valueMap = {
            "VIOLATION": {
    ...
    this.report = async (report1) => {
        ...
        let allRulesets = await aChecker.getRulesets();
        let myThis = this;
        let myRulesets = allRulesets.filter((function (rs) { 
            return rs.id === myThis.accessibilityGuidelines }));
        report.ruleset = myRulesets[0];
        ...
        var xlsx_props = {
            report: report,
            rulesets: await aChecker.getRulesets(),
        ...
        let myScanData = new MultiScanData(this.Config, aChecker)
    ...
    this.savePageResults = (report) => {
        ...
        for (const result of report.results) {
            this.resultStr += `${toCSV(report.label)},${toCSV(result.level)},${toCSV(result.ruleId)},${toCSV(result.message)},${toCSV(result.path.dom)},${toCSV(aChecker.getHelpURL(result))}\n`
        }
     ...
     * @memberOf this
     */
    this.writeObjectToFile = (fileName, content) => {
     ...
     * @memberOf this
     */
    this.addResultsToGlobal = (results) => {
     ...
     * @memberOf this
     */
    this.addToSummaryCount = (pageCount) => {
