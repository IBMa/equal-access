...
When elements are assigned a WAI-ARIA `role`, WAI-ARIA 1.2 specifies which attributes are valid for that role. If invalid WAI-ARIA attributes are used, assistive technologies may not be able to interpret and operate the element accurately for users.
...
* OR, change the `role` value to a role that supports the intended behavior of the element, using the WAI-ARIA 1.2 specification as a guide.
...
[WC3 WAI-ARIA 1.2 states and properties](https://www.w3.org/WAI/PF/aria-1.2/states_and_properties)
