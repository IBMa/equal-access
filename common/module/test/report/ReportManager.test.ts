...
import { IFS } from "../../src/api-ext/IFS";
import { IConfigInternal, eRuleLevel } from "../../src/config/IConfig";
import { ReporterManager } from "../../src/report/ReportManager";
...
const expected = {
    ...
    "results.csv": readFileSync(join(__dirname, "results_expected.csv")).toString()
}
...
test("Generates reports", async () => {
    ...
    const reportManager = new ReporterManager(new FakeFS, myConfig, rulesets);
    ...
    reportManager.addEngineReport(1686937348053, url, "My Report 1", "report1", report1, baseline1);
    ...
    reportManager.addEngineReport(1686937348153, url2, "My Report 2", "report2", report2, baseline2);

    await reportManager.generateSummaries(1686945479477);
