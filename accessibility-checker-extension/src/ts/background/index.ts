...
BackgroundMessaging.addListener("SEND_TABBING_DATA_TO_BACKGROUND", async (message: any) => {
    ...
    console.log(message)
    await BackgroundMessaging.sendToPanel("SEND_TABBING_DATA_TO_PANEL", message);
...
BackgroundMessaging.addListener("SEND_TABSTOPS_TO_CONTEXT_SCRIPTS", async (message: any) => {
    console.log("Message SEND_TABSTOPS_TO_CONTEXT_SCRIPTS received in background")
    await BackgroundMessaging.sendToTab(message.tabId, "SEND_TABSTOPS_TO_CONTEXT_SCRIPTS", { tabId: message.tabId, tabURL: message.tabURL, tabStopsResults: message.tabStopsResults});
