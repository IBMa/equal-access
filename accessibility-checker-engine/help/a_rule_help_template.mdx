...
---
<!-- change the title to match the rule_ID -->
...
### The active message goes here _without tokens_ but with `backticks for formating code` and tokens **cannot** be used in mdx 
<!-- this message title is not used by the Checker, the browser extension uses the active messages: Fail_ or Potential_ message in ...nls/index.ts file instead -->
...
<!-- Checker does not display this message sub-title, the browser extension displays the 0: message in  ...nls/index.ts file -->
<!-- note that the messages the test files must also match the messages from the ...nls/index.ts file instead of here in mdx -->  
