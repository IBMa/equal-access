...
[IBM 4.1.2 Name, Role, Value](http://www.ibm.com/able/guidelines/ci162/name_role_value.html) | [W3C WAI-ARIA 1.1 Required Owned Elements](https://www.w3.org/TR/wai-aria/#mustContain)
...
For example, the following element uses the `"radiogroup"` role. The `"radiogroup"` role requires the child element with `role="radio"` set in a manner that represents the checked or unchecked state of the radio button.
