...
### An element with ARIA `'role'` does not contain or own at least one child element with a valid ARIA role
...
If an ARIA `'role'` attribute is specified for an element, any ARIA child elements required for that role must also be present. 
Child elements' native ARIA roles satisfy the requirement - the role does not have to be explicitly set. 
Without the required child elements, assistive technologies may not be able to accurately represent or interact with the element.
