...
receives focus, the focus should be set with the text `<input>` element. Unless using a `dialog`
...
* Unless the `has-popup` attribute value is `dialog`, set the active descendant focus using the `aria-activedescendant` attribute only with the combobox’s `<input>` element.
