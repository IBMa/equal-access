 ...
 * **Or**, if the `<canvas>` element contains a simple drawing, provide a `role="img"` with the `<canvas>` element with a label. If the label is not sufficient to make the drawing accessible, also provide a description (`aria-describedby="foo"`);
...
This example shows the use of the `<canvas>` element with its content exposed as an image with a ARIA `'role'` attribute, a label, and a description.
