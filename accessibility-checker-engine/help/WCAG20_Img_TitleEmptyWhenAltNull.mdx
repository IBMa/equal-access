---
title: "Accessibility Checker Rule Help: WCAG20_Img_TitleEmptyWhenAltNull"
---
import "../../../styles/ToolHelp.scss"
import { Tag } from "carbon-components-react";

<div className="toolHelp">
<Row>
<Column colLg={5} colMd={3} colSm={4} className="toolLeft">

## Who does this affect?

* Blind people using screen readers

## Why is this important?

When an image is decorative and has an empty `alt` attribute, the `title` attribute should not be included so the image is ignored by assistive technologies.

</Column>
<Column colLg={11} colMd={5} colSm={4} className="toolMain">

<div className="level-violation">
Violation
</div>

## The image `alt` text attribute is empty, but the `title` attribute is not empty
When the image `alt` text attribute is empty, the `title` attribute must also be empty

[IBM 1.1.1 Non-text content](http://www.ibm.com/able/guidelines/ci162/non_text_content.html) | [WCAG 2.1 technique H67](https://www.w3.org/TR/WCAG20-TECHS/H67)

<div id="elementLocation">
</div>

## What to do

Use a null `alt` text attribute and no `title` attribute on an image that assistive technologies should ignore.

For example:
```<img src="squiggle.gif" width="20" height="20" alt="" />```


</Column>
</Row>
</div>

export default ({ children, _frontmatter }) => (<React.Fragment>{children}</React.Fragment>)