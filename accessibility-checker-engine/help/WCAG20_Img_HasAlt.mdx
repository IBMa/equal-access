...
* If the image conveys meaning, use the `'alt'` attribute with the `<img>` element to provide a short description that serves the same purpose as the image. If the image contains important words, include them in the short description (e.g. `<img alt="submit">`);
