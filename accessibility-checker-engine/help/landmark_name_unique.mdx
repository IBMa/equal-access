...
### Multiple landmark roles with the same `'role'` attribute are not disambiguated with a unique label or different parent  

<div id="locLevel"></div>

Multiple landmarks with the same `'role'` should have a unique `'aria-labeledby'` or `'aria-label'` or be nested in different parents
...
and other ATs will have difficulty identifying or disambiguating between them 
...
If multiple landmarks have the same role, such as `role="contentinfo"` or `role="navigation"`, the landmarks should be disambiguated:  
* Use unique `'aria-labeledby'` values to disambiguate landmarks with the same `'role'`
* **Or**, use unique `'aria-label'` values to disambiguate landmarks with the same `'role'`
* **Or**, nest landmarks in different parent regions to disambiguate those with the same `'role'`
