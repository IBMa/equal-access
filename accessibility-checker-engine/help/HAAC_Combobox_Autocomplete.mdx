...
### The combobox has the `'aria-autocomplete'` attribute set with an element that isn't a text input

<div id="locLevel"></div>

A combobox that supports autocompletion behavior must have the `'aria-autocomplete'` attribute only with its text input element
...
Note, example code includes material copied from or derived from [ARIA 1.1 Authoring Practices](https://www.w3.org/TR/wai-aria-practices-1.1/examples/combobox/aria1.1pattern/listbox-combo.html). Copyright © [2013-2017] W3C® (MIT, ERCIM, Keio, Beihang).
