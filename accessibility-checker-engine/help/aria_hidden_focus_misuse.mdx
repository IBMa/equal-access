...
Using either `role="presentation"` or `aria-hidden="true"` with a focusable element will result in some users focusing on "nothing".
...
Note: Using `aria-hidden="false"` with a descendant of an element with `aria-hidden="true"` does not expose that element.
