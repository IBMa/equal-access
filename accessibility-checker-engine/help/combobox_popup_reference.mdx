...
an attribute `aria-owns` or `aria-controls` referencing its `id`. When the value of the `aria-owns` or 
the `aria-controls` attribute of the expanded combobox does not reference a valid popup `id`, assistive 
