---
title: "Accessibility Checker Rule Help: Valerie_Caption_HasContent"
---
import "../../../styles/ToolHelp.scss"
import { Tag } from "carbon-components-react";

<div className="toolHelp">
<Row>
<Column colLg={5} colMd={3} colSm={4} className="toolLeft">

## Who does this affect?

 * Blind people using screen readers

## Why is this important?

The caption for a table is an identifier and acts like a title or heading for the table. The `<caption>` element allows screen readers to navigate directly to the caption to get the description of the table.

</Column>
<Column colLg={11} colMd={5} colSm={4} className="toolMain">

<div className="level-violation">
Violation
</div>

## A `<table>` element does not have a `<caption>` element that contains descriptive text
A `<table>` element must have a `<caption>` element that contains descriptive text

[IBM 1.3.1 Info and Relationships](http://www.ibm.com/able/guidelines/ci162/info_and_relationships.html) | [WCAG 2.1 technique H39](https://www.w3.org/WAI/WCAG21/Techniques/html/H39)

<div id="elementLocation">
</div>

## What to do

* Add descriptive text to the `<caption>` element.

For example:
```
<table id="data_table1" border="1">
<caption>Boys and Girls in Elementary School Classes</caption>
	<tr>
		<th scope="col">Class</th>
		<th scope="col"># of Boys</th>
		<th scope="col"># of Girls</th>
	</tr>
</table>
```

</Column>
</Row>
</div>

export default ({ children, _frontmatter }) => (<React.Fragment>{children}</React.Fragment>)