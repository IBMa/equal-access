...
Within an `application` region (or area), only focusable elements are accessible to users of some assistive technologies. Therefore, to ensure access to any non-decorative static text or image content that does not receive focus by default, it must be implemented in an accessible way.
...
## Check non-decorative static text or image content within the `application` role has keyboard focus
Non-decorative static text or image content within the `application` role must receive keyboard focus
...
* Associate the content with a focusable element using the `aria-labelledby` or `aria-describedby` attributes;
