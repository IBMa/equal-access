...
current value or supports editing, so people who use assistive technologies are able accurately 
...
* Unless the `has-popup` atttribute value is `dialog`, set the initial DOM focus only on the combobox’s `<input>` element.
