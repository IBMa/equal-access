...
Tabbable focus for the combobox must be allowable only with the text input, except when using a dialog popup
...
receives focus, the focus should be set with the text `<input>` element that can either display the 
...
* Unless the `'has-popup'` attribute value is `"dialog"`, set the initial DOM focus only with the combobox’s `<input>` element.
