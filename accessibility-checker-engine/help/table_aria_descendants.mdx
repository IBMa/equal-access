...
### An explicit ARIA `'role'` is not valid for the element within an ARIA role per the ARIA in HTML specification
...
ARIA `'role'` with an element as defined in the ARIA specification allows assistive 
