 ...
 * **Or**, if the component is not keyboard accessible, implement `onkeydown` or `onkeypress` keyboard handlers with the element itself using [ARIA practices](https://www.w3.org/TR/wai-aria-practices-1.2);
 * **Or**, if the inaccessible component has required children, implement `onkeydown` or `onkeypress` keyboard handlers with all the required children (example below) or a grandchild of each required child;
