...
### `<area>`  element within an image map has no text alternative

<div id="locLevel"></div>

`<area>` elements within an image map must have a text alternative
...
Use an `'alt'` attribute with each `<area>` element to provide a short description that serves the same purpose as the `<area>` within the image map.

Example:

<CodeSnippet type="multi" light={true}>&lt;img src="welcome.gif" usemap="#map1" alt="Areas in the library. Select an area for more information with that area." /&gt;
