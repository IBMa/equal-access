...
### HTML5 attribute is in conflict with the associated ARIA attribute used with an input element

<div id="locLevel"></div>

HTML5 attributes must not conflict with the associated ARIA attribute used with an input element
...
Using both an HTML5 attribute and the associated ARIA attribute with the same semantic meaning with an `<input>` element can cause a conflict that allows one of the attributes to override the other. This can lead to unexpected and confusing assistive technology behavior.
