...
People with visual impairment and others who use assistive technologies.
 ...
 * Place a single `<legend>` as the first element in the `<fieldset>` group. The legend must contain a brief description of the related fields enclosed by the `<fieldset>`. For example:
