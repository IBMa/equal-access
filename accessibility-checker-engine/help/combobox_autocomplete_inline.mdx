...
value of `"inline"` and must have the `'aria-autocomplete'` attribute only with its text input element
...
Note, example code includes material copied from or derived from [ARIA practices](https://w3c.github.io/aria-practices/examples/combobox/combobox-autocomplete-list.html). Copyright © [2013-2017] W3C® (MIT, ERCIM, Keio, Beihang).
